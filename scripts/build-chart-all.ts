// build-chart-all - every native String operation vs its vstring counterpart,
// built from the chart01 template: the json-as `createBarChart` / `generateChart`
// lib, grouped bars with native and vstring (SIMD) side by side per operation.
//
// Metric is speedup vs native (native = the 1× baseline bar, vstring = how many
...
for (const suite of Object.keys(PAYLOADS)) {
  chartData[suite] = [
    ...
    bar(speedup(suite), "vstring (SIMD)"),
  ];
}

const config = createBarChart(chartData, PAYLOADS, {
  title: "vstring - every String op vs its vstring counterpart",
  yLabel: "Speedup vs native (× - higher is faster)",
  xLabel: "",
  datasetLabels: ["native (baseline)", "vstring (SIMD)"],
  // native = strawberry red baseline, vstring = pacific blue (SIMD).
