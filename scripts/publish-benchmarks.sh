...
if [[ "$RUN_BENCHES" == "1" ]]; then
  ...
  bun ./scripts/build-chart-all.ts
fi
...
(
  ...
  git config user.name "${GIT_AUTHOR_NAME:-$(git config --get user.name || echo vstring)}"
  git config user.email "${GIT_AUTHOR_EMAIL:-$(git config --get user.email || echo vstring@example.com)}"
...
echo "README pinned to https://raw.githubusercontent.com/JairusSW/vstring/refs/heads/${DOCS_BRANCH}/charts/${DEST}/"
