...
function packedSpanOwner(name, context) {
  return (
...
function rewriteExpression(expression, expected, context, signatures) {
  ...
  if (expression instanceof BinaryExpression) {
    ...
    const equality =
      expression.operator === 76 ||
      expression.operator === 78 ||
      expression.operator === 77 ||
      expression.operator === 79;
    if (equality) {
      const directView =
      ...
      if (directView) {
        ...
        if (
          ...
          return staticViewCall(
            expression.operator === 77 || expression.operator === 79
              ? "notEqualsSpan"
              : "equalsSpan",
            parts.receiver,
            [
