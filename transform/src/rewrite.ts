...
function viewCallParts(expression: CallExpression): {
  method: string;
  receiver: Expression;
  args: Expression[];
} | null {
...
function expressionRepresentation(
  ...
  if (
    "literalKind" in expression &&
    expression.literalKind === LiteralKind.Template
  ) {
    return "native";
  }
...
export function rewriteDeclarationBoundaries(
